let events2017 = [
    {
        date: '2017-01-08',
        place: 'Ленинградская обл., п Цвелодубово, ДОЛ "Голубое озеро"',
        name: 'Рождественские старты - 2017',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170107_08/170107_res.htm',
        info: 'В связи с отсутствием достаточного снежного покрова соревнования в 2017 году проводятся БЕГОМ.',
        owner: 'NW',
        o_site: '170107_08'
    },
    {
        date: '2017-01-10',
        place: 'Ленинградская обл., Выборгский р-он, п.Цвелодубово',
        name: 'Зимний сбор в Цвелодубово-2017',
        type: 'VELO',
        reg: 'http://sportident.ru/entry/event_info.php?id=1898',
        info: 'Зимний сбор в Цвелодубово-2017',
        o_site: '170103_sbor'
    },
    {
        date: '2017-01-14',
        place: 'Орехово',
        name: 'Староновогодний рогейн',
        type: 'ROGAINE',
        reg: 'http://www.x-race.info/calendar/89155/request/',
        res: 'http://www.multsport.ru/starts/oldny-rogaine-2017/results.html',
        start: 'MULTSPORT',
        o_site: '170114'
    },
    {
        date: '2017-01-14',
        name: 'Тренировка в Вартемягах',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/',
        res: 'https://o-site.spb.ru/_races/170114_tren/170114_res.htm',
        o_site: '170114_tren'
    },
    {
        date: '2017-01-15',
        place: 'ж.д. Орехово, база отдыха "ПАН"',
        name: 'Кубок СПб по спортивному ориентированию на лыжах',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170115/170115_res.pdf',
        info: '15 января база "ПАН" Спринт',
        o_site: '170115'
    },
    {
        date: '2017-01-21',
        endDate: '2017-01-22',
        place: 'ж.д. Орехово, база отдыха "ПАН"',
        name: 'Спартакиада учащихся России - Санкт-Петербург',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170121-22_SP/170121_res.htm',
        o_site: '170121-22_SP'
    },
    {
        date: '2017-01-27',
        endDate: '2017-01-29',
        place: 'Будогощь',
        name: 'Чемпионат и первенство ЛО; зимний GRAND PRIX среди ветеранов',
        type: 'SKI',
        res: 'https://o-site.spb.ru/_races/17012729/170127_rez.htm',
        owner: 'SFSO_LO',
        o_site: '17012729'
    },
    {
        date: '2017-01-28',
        place: 'Кавголово, база ВИФК',
        name: 'Тренировка на ВИФКе',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=681',
        res: 'https://o-site.spb.ru/_races/170128_Sh/170128_res.htm',
        o_site: '170128_Sh'
    },
    {
        date: '2017-02-11',
        name: 'Мемориал А.Волкова',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170211/170211_res.htm',
        info: '11.02.2017 в ЗЦ "Зеркальный" на полигоне за магазином и железной дорогой будут проведены соревнования "Мемориал А.Волкова" в дисциплине "лыжная гонка-маркированная трасса" по группам М10, Ж10, М12, Ж12, М14, Ж14, М17, Ж17, М21, Ж21, М40, Ж40, М50, Ж50, М60, Ж60, М70, Ж70, М80, Ж80',
        o_site: '170211'
    },
    {
        date: '2017-02-13',
        place: 'ЗЦ Зеркальный',
        name: 'ТМ в Зеркальном',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170203-13/170212_res.htm',
        info: 'С 03 по 13 февраля 2017 г. в ЗЦ "Зеркальный" пройдет традиционная смена детских коллективов по спортивному ориентированию и спортивному туризму Санкт-Петербурга.',
        o_site: '170203-13'
    },
    {
        date: '2017-02-18',
        name: '58-й зимний ч-т и п-во СПб ЛГ-общий старт',
        type: 'SKI',
        res: 'https://o-site.spb.ru/_races/170218/170218_res.htm',
        o_site: '170218'
    },
    {
        date: '2017-02-19',
        name: '58-й зимний ч-т и п-во СПб ЛГ-спринт',
        type: 'SKI',
        res: 'https://o-site.spb.ru/_races/170219/170219_res.htm',
        o_site: '170219'
    },
    {
        date: '2017-02-23',
        place: 'Орехово',
        name: '58-й зимний ч-т и п-во СПб Маркированная трасса',
        type: 'SKI',
        res: 'https://o-site.spb.ru/_races/170223_ch/170223_res.htm',
        info: 'Дистанция: маркированная трасса (прокол в карте)',
        fmt: 'SFR-System',
        o_site: '170223_ch'
    },
    {
        date: '2017-02-25',
        place: 'Орехово',
        name: 'Чемпионат СПб среди студентов',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=696',
        res: 'https://o-site.spb.ru/_races/170225_VUZ/170225_res.htm',
        owner: 'SFSO_SPB',
        o_site: '170225_VUZ'
    },
    {
        date: '2017-02-26',
        name: '58-й зимний ч-т и п-во СПб ЛГ-классика',
        type: 'SKI',
        res: 'https://o-site.spb.ru/_races/170226_ch/170226_res.htm',
        o_site: '170226_ch'
    },
    {
        date: '2017-03-04',
        place: 'п.Кротово (оз.Вуокса)',
        name: 'Рогейн - Ice Trail 2017',
        type: 'ROGAINE',
        fmt: '6ч и 24ч',
        o_site: '170304'
    },
    {
        date: '2017-03-05',
        name: '58-й зимний ч-т и п-во СПб ЛГ-ЛОНГ',
        type: 'SKI',
        res: 'https://o-site.spb.ru/_races/170305/170305_res.htm',
        o_site: '170305'
    },
    {
        date: '2017-03-08',
        name: '58-й зимний ч-т и п-во СПб ЛГ-эстафета',
        type: 'SKI',
        res: 'https://o-site.spb.ru/_races/170308/170308_res.htm',
        info: '– Чемпионат Санкт-Петербурга: лыжная гонка – эстафета-3 чел. Допускается формирование команд из представителей различных спортивных коллективов или личников. - Первенство Санкт-Петербурга среди юношей, юниоров МЖ 12, МЖ14, МЖ17, МЖ20: лыжная гонка – эстафета-3 чел. Допускается формирование команд из представителей различных спортивных коллективов. Данные команды выступают внезачета. - Первенство Санкт-Петербурга среди ветеранов, группы МЖ35, МЖ45, МЖ55, МЖ65: лыжная гонка – эстафета -2 чел. После окончания регистрации спортсмены не прошедшие мандатную комиссию и заявившиеся в эстафетные команды будут удалены из заявки.',
        o_site: '170308'
    },
    {
        date: '2017-03-12',
        place: 'Нижний (отдельный) парк г. Пушкина',
        name: 'Весёлый Ветер',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=706',
        res: 'https://o-site.spb.ru/_races/170312/170312_res.htm',
        photo: 'http://vk.com/album-9087078_242268462',
        o_site: '170312'
    },
    {
        date: '2017-03-12',
        name: 'На приз газеты Всеволожские вести',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/',
        res: 'https://o-site.spb.ru/_races/17031112/170311_rez.htm',
        start: 'VSEV_VESTI',
        o_site: '17031112'
    },
    {
        date: '2017-03-19',
        name: 'Sprint Camp SPb 2017',
        type: 'ORIENT',
        reg: 'http://sportident.ru/entry/event_info.php?id=2026',
        res: 'https://o-site.spb.ru/_races/17031719/170317_rezSC.htm',
        o_site: '17031719'
    },
    {
        date: '2017-03-19',
        place: 'Болотная станция (Ильичево)',
        name: 'Весенняя тропа от FoxTeam',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=710',
        res: 'https://o-site.spb.ru/_races/170319_FoxTeam/170319_rezFT.htm',
        o_site: '170319_FoxTeam'
    },
    {
        date: '2017-04-01',
        name: 'Sprint Tour SPb 2017: 12 этап',
        type: 'ORIENT',
        reg: 'http://reg.o-time.ru/step1.php?event=17092',
        info: 'Серия тренировочных стартов в дисциплине "спринт"',
        start: 'ST',
        o_site: '170412_ST',
        endDate: '2017-10-31'
    },
    {
        date: '2017-04-01',
        name: 'Кузьмоловские старты',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/2017_KS/171005_res.htm',
        info: 'Серия тренировочных стартов в окрестностях поселка Кузьмоловский',
        o_site: '2017_KS',
        endDate: '2017-10-31'
    },
    {
        date: '2017-04-08',
        name: 'II Весенние капели, 3-й этап',
        type: 'ORIENT',
        res: 'http://o-site.spb.ru/_races/170326_VK/170326_res.htm',
        info: 'Серия тренировочных стартов: 25.03.17 Песочная, 1-й этап: результаты 02.04.17 Комарово, 2-й этап: результаты',
        o_site: '170326_VK'
    },
    {
        date: '2017-04-08',
        place: 'Эколого-биологический центр на Крестовском острове, Крестовский пр. 19',
        name: 'Вечер встречи ориентировщиков, награждение по итогам зимнего сезона',
        type: 'ORIENT',
        info: 'Традиционный вечер встречи по итогам зимнего сезона 2017. Организатор: Федерация спортивного ориентирования Санкт-Петербурга Проходит при поддержке: Санкт-Петербургского городского Дворца творчества юных Список призёров Чемпионата и первенств Санкт-Петербурга, Список призёров финального этапа Снежной Тропы. Список победителей Мемориала Андрея Волкова.',
        o_site: '170408'
    },
    {
        date: '2017-04-09',
        name: 'Всеволожская тропа 2017 3 этап',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/',
        res: 'https://o-site.spb.ru/_races/170223_VT/170409_rez.htm',
        o_site: '170223_VT'
    },
    {
        date: '2017-04-12',
        place: 'школа №529, Петергоф, Разводная ул., д. 27',
        name: 'Спортивное ориентирование - в школу (Петродворцовый р-н)',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170412/170412_res_lab.htm',
        photo: 'https://o-site.spb.ru/_races/170412/170412_res_foto.htm',
        info: 'Проект Федерации Спортивного Ориентирования Санкт-Петербурга, направленный на популяризацию спортивного ориентирования среди школьников. Проходит при поддержке: Комитета по молодежной политике и взаимодействию с общественными организациями Санкт-Петербурга, Комитета по образованию Санкт-Петербурга, ОАО "Фацер"',
        owner: 'SFSO_SPB',
        o_site: '170412'
    },
    {
        date: '2017-04-14',
        endDate: '2017-04-15',
        name: 'Спринты от FoxTeam',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=733',
        res: 'https://o-site.spb.ru/_races/170314-15/170414_split_FT.htm',
        info: 'Спринты от команды FoxTeam',
        o_site: '170314-15'
    },
    {
        date: '2017-04-19',
        place: 'ГБОУ школа № 604 по адресу: г. Пушкин, пос. Шушары, Славянка, ул. Изборская д.4 кор.2',
        name: 'Спортивное ориентирование - в школу (Пушкинский р-н)',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170419/170419_res_lab.htm',
        photo: 'https://o-site.spb.ru/_races/170419/170419_res_foto.htm',
        info: 'Проект Федерации Спортивного Ориентирования Санкт-Петербурга, направленный на популяризацию спортивного ориентирования среди школьников. Проходит при поддержке: Комитета по молодежной политике и взаимодействию с общественными организациями Санкт-Петербурга, Комитета по образованию Санкт-Петербурга, ОАО "Фацер"',
        owner: 'SFSO_SPB',
        o_site: '170419'
    },
    {
        date: '2017-04-23',
        name: 'Сосновоборские дюны - 2017',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170423/170422_res.htm',
        info: '22 апреля - Спринт с 15.00 23 апреля - Классика с 12.00',
        owner: 'YUVENTA',
        o_site: '170423'
    },
    {
        date: '2017-04-26',
        place: 'Территория школы-интерната №9 – ул.Старцева д.7',
        name: 'Трейл-ориентирование - в школу',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170426/170426_res.pdf',
        photo: 'http://vk.com/album-51571395_243769294',
        info: 'Проект Федерации Спортивного Ориентирования Санкт-Петербурга, направленный на популяризацию спортивного ориентирования среди школьников. Проходит при поддержке: Комитета по молодежной политике и взаимодействию с общественными организациями Санкт-Петербурга, Комитета по образованию Санкт-Петербурга, ОАО "Фацер"',
        owner: 'SFSO_SPB',
        o_site: '170426'
    },
    {
        date: '2017-04-29',
        place: 'Медное оз',
        name: 'Weekly Orient',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=743',
        res: 'https://o-site.spb.ru/_races/170429_WOr/170429_res.htm',
        o_site: '170429_WOr'
    },
    {
        date: '2017-04-29',
        name: 'Горный кросс - 55',
        type: 'ROGAINE',
        reg: 'http://x-race.info/calendar/92112/request/',
        res: 'https://o-site.spb.ru/_races/170429/170429GK_res.pdf',
        start: 'GORNY_CROSS',
        o_site: '170429'
    },
    {
        date: '2017-04-30',
        name: 'Первенство СДЮСШОР №2',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170430-0501_gdtu/170430_res.htm',
        info: '30 апреля - кросс классика, 01 мая - кросс спринт.',
        o_site: '170430-0501_gdtu'
    },
    {
        date: '2017-04-30',
        name: 'Соревнования памяти Ю.А.Березюка',
        type: 'ORIENT',
        reg: 'http://orgeo.ru/event/4112',
        info: '3-и традиционные Соревнования памяти Юрия Александровича Березюка. Кубок города Кириши по спортивному ориентированию. Кубок Профкома ООО «ПО Киришинефтеоргсинтез»',
        o_site: '170514'
    },
    {
        date: '2017-05-01',
        name: 'УТС в ОК Университетский',
        type: 'ORIENT',
        info: 'Специализированная детско-юношеская спортивная школа олимпийского резерва № 2 Санкт-Петербургского городского Дворца творчества юных организовывают тренировочное мероприятие по подготовке к первенствам Санкт-Петербурга и России по спортивному ориентированию бегом с 28 апреля по 01 мая 2017 года на базе оздоровительного комплекса «Университетский» Ленинградской области, Выборгского района, Полянской волости.',
        o_site: '170428-0501'
    },
    {
        date: '2017-05-01',
        name: 'Летний Кубок СПб и ЛО',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=755',
        info: 'Многоэтапные лично-командные соревнования по спортивному ориентированию «Кубок Санкт-Петербурга и Ленинградской области» проводятся Федерациями спортивного ориентирования Санкт-Петербурга и Ленинградской области с целью развития клубного движения и выявления сильнейших спортсменов-ориентировщиков и сильнейших клубов спортивного ориентирования СПб и ЛО. 1-й этап 27.05 Чемпионат и Первенство СПб, спринт. Выборг. Рейтинг. 2-й этап 28.05 Чемпионат и Первенство СПб, классика. Выборг. Рейтинг. 3-й этап 03.09 Чемпионат и Первенство СПб, выбор. Цвелодубово. Рейтинг. 4-й этап 24.09 Чемпионат и Первенство СПб, марафон / лонг-общий старт. Яппиля. Рейтинг. 5-й этап 01.10 Чемпионат и Первенство СПб, эстафета. Токсово. Рейтинг. 6-й этап 08.10 Чемпионат и Первенство СПб, лонг. Петяярви. Рейтинг. 7-й этап 22.10 Финал Кубка, классика. Выборг. Рейтинг.',
        o_site: 'SPb_LO_Cup_2017',
        endDate: '2017-10-31'
    },
    {
        date: '2017-05-03',
        place: 'ДДЮТ Выборгского района, ул. Сантьяго-де-Куба, 4, корп.2',
        name: 'Спортивное ориентирование - в школу (Выборгский р-н)',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170503/170503_res_lab.htm',
        photo: 'https://o-site.spb.ru/_races/170503/170503_res_foto.xls',
        info: 'Проект Федерации Спортивного Ориентирования Санкт-Петербурга, направленный на популяризацию спортивного ориентирования среди школьников. Проходит при поддержке: Комитета по молодежной политике и взаимодействию с общественными организациями Санкт-Петербурга, Комитета по образованию Санкт-Петербурга, ОАО "Фацер"',
        owner: 'VYBORG',
        o_site: '170503'
    },
    {
        date: '2017-05-06',
        place: 'Воейково',
        name: 'Колтушские высоты - 2-й этап Кубка Всеволожского района по рогейну',
        type: 'ROGAINE',
        reg: 'http://sport-orient.ru/products/sorevnovaniya-po-rogeynu/kvr-2017-vtoroy-etap',
        res: 'https://o-site.spb.ru/_races/170506/170506_res.htm',
        o_site: '170506'
    },
    {
        date: '2017-05-06',
        name: 'Кубок Белых Ночей - 2016/17',
        type: 'ORIENT',
        res: 'http://o-site.spb.ru/_races/2017WN/161009_res.htm',
        info: 'Серия тренировочных стартов, организованная клубом Белые Ночи. 1 этап 09.10.2016 садоводство "Топограф" результаты сплиты 2 этап 26.11.2016 Воейково результаты сплиты 3 этап 10.12.2016 Ильичёво результаты сплиты 4 этап 05.02.2017 Орово результаты сплиты 5 этап 12.02.2017 карьер Красная Горка результаты сплиты 6 этап 19.02.2017 Охта-парк результаты сплиты 7 этап 24.02.2017 Вуолы результаты сплиты 8 этап 05.03.2017 Дюны результаты сплиты 9 этап 12.03.2017 Малиновая гора результаты сплиты 10 этап 19.03.2017 Голубая Дача результаты сплиты 11 этап 25.03.2017 Воейково результаты сплиты 12 этап 02.04.2017 Матокса результаты сплиты 13 этап 09.04.2017 садоводство "Топограф" результаты сплиты 14 этап 01.05.2017 Петяярви результаты сплиты 15 этап 06.05.2017 Лемболово результаты сплиты',
        owner: 'WN',
        o_site: '2017WN'
    },
    {
        date: '2017-05-06',
        name: 'Спринты от FoxTeam',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=746',
        res: 'https://o-site.spb.ru/_races/170506FT/170506_splits_1.htm',
        o_site: '170506FT'
    },
    {
        date: '2017-05-07',
        name: 'Классика от FoxTeam',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=747',
        res: 'https://o-site.spb.ru/_races/170507FT/170507_res.htm',
        o_site: '170507FT'
    },
    {
        date: '2017-05-09',
        place: 'Орехово',
        name: 'Соревнования, посвящённые Дню Победы',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=749',
        res: 'https://o-site.spb.ru/_races/170509/170509_res.htm',
        o_site: '170509'
    },
    {
        date: '2017-05-10',
        place: 'г. Пушкин, Баболовский парк',
        name: 'Царскосельский Азимут',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=738',
        res: 'https://o-site.spb.ru/_races/170426_CA/170510_res.htm',
        o_site: '170426_CA'
    },
    {
        date: '2017-05-13',
        endDate: '2017-05-14',
        name: 'Чемпионат и Первенство ЛО',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/17042901/17051314_res_official.zip',
        owner: 'SFSO_LO',
        o_site: '17042901'
    },
    {
        date: '2017-05-17',
        place: 'школа № 69 по адресу: Санкт-Петербург, проспект Культуры, дом 27, корп.3, литер А',
        name: 'Спортивное ориентирование - в школу (Калининский район)',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170517/170517_res_lab.htm',
        photo: 'https://o-site.spb.ru/_races/170517/170517_res_foto.htm',
        info: 'Проект Федерации Спортивного Ориентирования Санкт-Петербурга, направленный на популяризацию спортивного ориентирования среди школьников. Проходит при поддержке: Комитета по молодежной политике и взаимодействию с общественными организациями Санкт-Петербурга, Комитета по образованию Санкт-Петербурга, ОАО "Фацер"',
        owner: 'SFSO_SPB',
        o_site: '170517'
    },
    {
        date: '2017-05-19', endDate: '2017-05-21', name: 'Чемпионат Прибалтики', type: 'ORIENT', o_site: '17051921'
    },
    {
        date: '2017-05-20',
        place: 'Парк 300-летия Санкт-Петербурга, Приморский пр., д. 74',
        name: 'Российский Азимут 2017 - Санкт-Петербург',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=752',
        res: 'https://o-site.spb.ru/_races/170520_SPb/170520_res.htm',
        photo: 'https://o-site.spb.ru/_races/170520_SPb/ https://cloud.mail.ru/public/K4Gk/7yX9j7YRY',
        owner: 'SFSO_SPB',
        o_site: '170520_SPb'
    },
    {
        date: '2017-05-20',
        place: '«Парк 300-летия Санкт-Петербурга» (Санкт-Петербург, Приморский проспект, 74)',
        name: 'Спартакиады Санкт-Петербурга по спортивному ориентированию',
        type: 'ORIENT',
        o_site: '170520_spartakiada'
    },
    {
        date: '2017-05-20',
        endDate: '2017-05-21',
        name: 'Водный рогейн',
        type: 'ROGAINE',
        reg: 'http://www.multsport.ru/starts/canoe-rogaine-2017/registration.html',
        fmt: 'Рогейн 8ч, рогейн 16ч.',
        start: 'MULTSPORT',
        o_site: '17052021'
    },
    {
        date: '2017-05-21',
        place: 'садоводство "Топограф"',
        name: 'Памяти друзей',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=754',
        res: 'https://o-site.spb.ru/_races/170520_Az/170521_res.htm',
        o_site: '170520_Az'
    },
    {
        date: '2017-05-21',
        name: 'Российский азимут 2017 - Ленинградская область',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=753',
        res: 'https://o-site.spb.ru/_races/170521/170521_rez_RA.htm',
        owner: 'SFSO_LO',
        o_site: '170521'
    },
    {
        date: '2017-05-21',
        place: 'Сестрорецкие Дюны',
        name: 'Чемпионат и п-во СПб по велоориентированию (классика)',
        type: 'VELO',
        res: 'https://o-site.spb.ru/_races/170521_velo/170521_rez.htm',
        owner: 'SFSO_SPB',
        o_site: '170521_velo'
    },
    {
        date: '2017-05-24',
        place: 'Политехнический Университет',
        name: 'Всемирный День Ориентирования',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170524/170524_res.htm',
        o_site: '170524'
    },
    {
        date: '2017-05-27',
        place: 'Выборг',
        name: '59-й Чемпионат и первенство СПб - спринт',
        type: 'ORIENT',
        reg: 'http://sportident.ru/entry/event_info.php?id=2029',
        res: 'http://www.o-site.spb.ru/_races/170427-28_VT/170527_split_VT.htm',
        owner: 'SFSO_SPB',
        o_site: '170527',
        reskeep: 255
    },
    {
        date: '2017-05-27',
        endDate: '2017-05-28',
        name: 'Выборгская тройка-2017',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170427-28_VT/170527_rez_VT.htm',
        owner: 'VYBORG',
        o_site: '170427-28_VT'
    },
    {
        date: '2017-05-27',
        endDate: '2017-05-28',
        place: 'Кузнечное',
        name: 'ГорныйКросс-56',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170527R/170527R_res.pdf',
        fmt: 'рогейн БЕГ - 24 и 6ч, ВЕЛО - 5ч, кросс-трейл - 38 и 17км',
        o_site: '170527R'
    },
    {
        date: '2017-05-28',
        place: 'Выборг (ст. Возрождение)',
        name: '59-й Чемпионат и первенство СПб - классика',
        type: 'ORIENT',
        reg: 'http://sportident.ru/entry/event_info.php?id=2029',
        res: 'http://www.o-site.spb.ru/_races/170427-28_VT/170528_split_day.htm',
        owner: 'SFSO_SPB',
        o_site: '170528',
        reskeep: 253
    },
    {
        date: '2017-06-02',
        endDate: '2017-06-08',
        name: 'Неделя ориентирования «Белые Ночи на Воробьевом озере»',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru',
        res: 'https://o-site.spb.ru/_races/17060204/170602_res.htm',
        owner: 'WN',
        start: 'WN',
        o_site: '17060204'
    },
    {
        date: '2017-06-05',
        place: 'Сосново (60.612059, 30.235747)',
        name: 'Ночная тренировка от FoxTeam',
        type: 'ORIENT',
        o_site: '170605_NFT'
    },
    {
        date: '2017-06-08',
        name: 'Спринт в Приозерске',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=762',
        res: 'https://o-site.spb.ru/_races/170608/170602_split_1.htm',
        o_site: '170608'
    },
    {
        date: '2017-06-09',
        endDate: '2017-06-11',
        place: 'Ягодное',
        name: 'XXVII многодневка «Невский Азимут»',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=732',
        res: 'https://o-site.spb.ru/_races/17060911/170609_res.htm',
        start: 'NA',
        o_site: '17060911'
    },
    {
        date: '2017-06-10',
        name: 'Велосипедный рогейн в Моторном',
        type: 'ROGAINE',
        reg: 'http://docs.google.com/forms/d/e/1FAIpQLSd87dXZOK0xfe9U_oNFflJBXp19lGBYW-7nurB-ms8IK3jOkQ/viewform?c=0&w=1',
        fmt: 'Велосипедный рогейн 4 и 6ч.',
        o_site: '170610'
    },
    {
        date: '2017-06-12',
        place: 'пос.Ягодное',
        name: 'Вело-О на Фестивале «Яркий Мир»',
        type: 'VELO',
        res: 'http://o-site.spb.ru/_races/17061218/170612V_split.htm',
        info: 'Соревнования по ориентированию велокросс-классика на спортивном Фестивале «Яркий Мир»',
        start: 'YM',
        o_site: '170612v'
    },
    {
        date: '2017-06-12',
        endDate: '2017-06-18',
        place: 'п. Ягодное',
        name: 'Спортивный фестиваль «Яркий Мир»',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/17061218/170612R_res.pdf',
        start: 'YM',
        o_site: '17061218'
    },
    {
        date: '2017-06-17',
        name: 'Велорогейн 100x24, 3й этап Кубка Всеволожского р-на',
        type: 'ROGAINE',
        reg: 'http://www.o-reg.spb.ru/?id=766',
        o_site: '170617_rog'
    },
    {
        date: '2017-06-18',
        endDate: '2017-06-23',
        place: 'вблизи г. Каменногорск, район п.Пруды',
        name: 'Кубок Карельского Перешейка',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/20170518_23KKP/vseros_KKP.rar',
        start: 'KKP',
        o_site: '20170518_23KKP'
    },
    {
        date: '2017-06-23',
        endDate: '2017-06-25',
        place: 'Коробицино',
        name: 'XXV Мемориал Михаила Святкина',
        type: 'ORIENT',
        start: 'MMS',
        o_site: '17062325'
    },
    {
        date: '2017-07-01',
        name: 'Чемпионат и Первенство Санкт-Петербурга по велоориентированию, спринт',
        type: 'VELO',
        reg: 'http://o-reg.spb.ru/entry-list?id=764',
        res: 'https://o-site.spb.ru/_races/170701/170701_res.htm',
        owner: 'SFSO_SPB',
        o_site: '170701'
    },
    {
        date: '2017-07-02',
        name: 'Чемпионат и Первенство Санкт-Петербурга по велоориентированию, лонг',
        type: 'VELO',
        reg: 'http://o-reg.spb.ru/entry-list?id=765',
        res: 'https://o-site.spb.ru/_races/170702/170702_res.htm',
        owner: 'SFSO_SPB',
        o_site: '170702'
    },
    {
        date: '2017-07-08',
        place: 'Поляны',
        name: 'Мини-рогейн «Четверть Века»',
        type: 'ROGAINE',
        reg: 'http://o-reg.spb.ru/entry-list?id=771',
        res: 'https://o-site.spb.ru/_races/170708/results.htm',
        o_site: '170708'
    },
    {
        date: '2017-07-09',
        place: 'Петяярви. к востоку от ж/д',
        name: 'Перспектива, 1-й этап',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=773',
        res: 'https://o-site.spb.ru/_races/2017_Per/170709_res.htm',
        info: 'Серия из четырёх личных стартов',
        o_site: '2017_Per'
    },
    {
        date: '2017-07-15',
        name: 'ГорныйКросс-57',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170715R/170715R_res.pdf',
        fmt: '6 и 3 часа - бег, 5 часов - вело',
        o_site: '170715R'
    },
    {
        date: '2017-07-22',
        endDate: '2017-07-23',
        place: 'Мичуринское',
        name: 'Летнее первенство Белых Ночей',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=774',
        res: 'https://o-site.spb.ru/_races/17072223/170722_res.htm',
        owner: 'WN',
        o_site: '17072223'
    },
    {
        date: '2017-07-23',
        place: 'Токсово',
        name: 'Перспектива, 2-й этап',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=775',
        res: 'https://o-site.spb.ru/_races/170723/170723_res.htm',
        info: 'Серия из четырёх личных стартов',
        o_site: '170723'
    },
    {
        date: '2017-07-30',
        name: 'Ладожский ориентир',
        type: 'ORIENT',
        reg: 'http://sportident.ru/entry/event_info.php?id=3389',
        res: 'https://o-site.spb.ru/_races/17072930/170729_split.htm',
        o_site: '17072930'
    },
    {
        date: '2017-08-06',
        place: 'Первомайское',
        name: 'Перспектива, 3-й этап',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=776',
        res: 'https://o-site.spb.ru/_races/170806_Per/170806_res.htm',
        info: 'Серия из четырёх личных стартов',
        o_site: '170806_Per'
    },
    {
        date: '2017-08-12',
        endDate: '2017-08-13',
        place: 'оз. Смоляное',
        name: 'Летние выходные на Смоляном озере',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=778',
        res: 'https://o-site.spb.ru/_races/17081213/170812_res_.htm',
        info: 'Двухдневные соревнования. Проводит клуб Белые Ночи.',
        o_site: '17081213'
    },
    {
        date: '2017-08-26',
        name: 'History Race 2017',
        type: 'ORIENT',
        reg: 'http://www.x-race.info/calendar/103070/request/',
        res: 'https://o-site.spb.ru/_races/170826/170826_res.pdf',
        o_site: '170826'
    },
    {
        date: '2017-08-26',
        name: 'Перспектива, 4-й этап',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=782',
        res: 'https://o-site.spb.ru/_races/170826_Per/170826_res.htm',
        info: 'Серия из четырёх личных стартов',
        o_site: '170826_Per'
    },
    {
        date: '2017-08-27',
        place: 'район ж/д ст. Лемболово',
        name: 'Памяти Димы Фатидинова',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170827/170827_PDF_rez.htm',
        info: 'Ежегодные соревнования Памяти Димы Фатидинова',
        o_site: '170827'
    },
    {
        date: '2017-09-03',
        place: 'р-н Цвелодубово',
        name: '59-й Чемпионат и первенство СПб - выбор',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=783',
        res: 'https://o-site.spb.ru/_races/170903/170903_res_V.htm',
        owner: 'SFSO_SPB',
        o_site: '170903'
    },
    {
        date: '2017-09-09',
        endDate: '2017-09-10',
        place: 'пос. Ягодное',
        name: 'MatrasOFF Race',
        type: 'ORIENT',
        reg: 'http://x-race.info/calendar/102909/request/',
        info: 'приключенческая гонка MatrasOFF Race - 2017 кубок Всеволожского района по рогейну, 4 этап',
        fmt: 'Рогейн 6ч (бег, вело), приключенческая гонка (Hard, Lite)',
        o_site: '170909'
    },
    {
        date: '2017-09-10',
        place: 'п. Симагино',
        name: 'Классика Белых Ночей',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=786',
        res: 'https://o-site.spb.ru/_races/170910_BN/170910_res.pdf',
        info: '8-е традиционные соревнования на призы компании Триколор ТВ',
        owner: 'WN',
        o_site: '170910_BN'
    },
    {
        date: '2017-09-16',
        name: 'Вело-О на Велопитере',
        type: 'VELO',
        res: 'http://velopiter.spb.ru/club/fest17/mtbo-fin.htm',
        o_site: '170916'
    },
    {
        date: '2017-09-16',
        place: 'пос. Стеклянный',
        name: 'Чемпионат и первенство Ленинградской обл., эстафета',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=789',
        res: 'https://o-site.spb.ru/_races/170916_LO/170916_res_.htm',
        owner: 'SFSO_LO',
        o_site: '170916_LO'
    },
    {
        date: '2017-09-17',
        place: 'пос. Стеклянный',
        name: 'Чемпионат и первенство Ленинградской обл., классика',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=790',
        res: 'https://o-site.spb.ru/_races/170917_LO/170917_res.htm',
        owner: 'SFSO_LO',
        o_site: '170917_LO'
    },
    {
        date: '2017-09-22',
        endDate: '2017-09-25',
        name: 'Старты в Зеркальном 2017',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170922-25/170923_res.htm',
        info: '22-25 сентября в ЗЦ «Зеркальный» пройдет традиционная осенняя смена юных ориентировщиков города. В программе тренировки на местности и участие в соревнованиях «СТАРТЫ в ЗЕРКАЛЬНОМ» и Первенстве Санкт-Петербурга. Подробности в информационных материалах. Организаторы просят руководителей групп подтвердить количество путевок по тел. 89213182044',
        o_site: '170922-25'
    },
    {
        date: '2017-09-23',
        place: 'Полюстровский парк',
        name: 'Первенство Красногвардейского р-на',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170923/170923_res.pdf',
        o_site: '170923'
    },
    {
        date: '2017-09-23',
        place: 'Будогощь',
        name: 'Чемпионат и п-во ЛО по вело-О',
        type: 'VELO',
        res: 'https://o-site.spb.ru/_races/170923_velo/170923_res_velo.html',
        owner: 'SFSO_LO',
        o_site: '170923_velo'
    },
    {
        date: '2017-09-24',
        place: 'Яппиля',
        name: '59-й Чемпионат и первенство СПб - марафон',
        type: 'ORIENT',
        res: 'http://orgeo.ru/live/#/5260',
        owner: 'SFSO_SPB',
        o_site: '170924',
        reskeep: 236
    },
    {
        date: '2017-09-26',
        place: 'ГБОУ средняя школа №339 Невского района Санкт-Петербурга. Улица Дыбенко 12-2',
        name: 'Спортивное ориентирование - в школу (Невский район)',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170926/170926_res_lab.htm',
        info: 'Проект Федерации Спортивного Ориентирования Санкт-Петербурга, направленный на популяризацию спортивного ориентирования среди школьников. Проходит при поддержке: Комитета по молодежной политике и взаимодействию с общественными организациями Санкт-Петербурга, Комитета по образованию Санкт-Петербурга, ОАО "Фацер"',
        owner: 'SFSO_SPB',
        o_site: '170926'
    },
    {
        date: '2017-09-27',
        name: 'Осенний спринт',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=795',
        o_site: '170927'
    },
    {
        date: '2017-09-29',
        place: 'Осиновая роща',
        name: 'Вечерняя тренировка от FoxTeam',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170929/170929_split.htm',
        o_site: '170929'
    },
    {
        date: '2017-09-30',
        name: 'ГорныйКросс-59',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/170930R/170930R_res.pdf',
        fmt: 'рогейн: бегом 5 и 2 часа, вело - 4 часа',
        o_site: '170930R'
    },
    {
        date: '2017-09-30',
        place: 'Токсово',
        name: 'Золотые кольца (Свадебные-2017)',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=798',
        info: '43-е традиционные соревнования супружеских пар. Допускаются пары по интересам.',
        start: 'WEDDING',
        o_site: '170930_ZK'
    },
    {
        date: '2017-10-01',
        place: 'Токсово',
        name: '59-й Чемпионат и первенство СПб - эстафета',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/171001/171001_res.htm',
        owner: 'SFSO_SPB',
        o_site: '171001',
        reskeep: 232
    },
    {
        date: '2017-10-04',
        place: 'школа №226, Бухарестская улица, 33, корп.6 литера А',
        name: 'Спортивное ориентирование - в школу (Фрунзенский район)',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/171004/171004_res_lab.htm',
        photo: 'http://vk.com/fazer_russia?w=wall-51571395_4148&z=album-51571395_247911082',
        info: 'Проект Федерации Спортивного Ориентирования Санкт-Петербурга, направленный на популяризацию спортивного ориентирования среди школьников. Проходит при поддержке: Комитета по молодежной политике и взаимодействию с общественными организациями Санкт-Петербурга, Комитета по образованию Санкт-Петербурга, ОАО "Фацер"',
        owner: 'SFSO_SPB',
        o_site: '171004'
    },
    {
        date: '2017-10-04',
        place: 'Токсово',
        name: 'Старт памяти одноклубника Коли Иванова',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/171004_PPKI/171004_PPKI_rez.htm',
        o_site: '171004_PPKI'
    },
    {
        date: '2017-10-07',
        place: 'Полюстровский парк',
        name: 'Парковый спринт Золотая Осень',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/171007kr/171007_res.htm',
        owner: 'VYBORG',
        o_site: '171007kr'
    },
    {
        date: '2017-10-07',
        place: 'Зеленогорск',
        name: 'Чемпионат СПб по ориентированию среди ВУЗов',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=800',
        res: 'https://o-site.spb.ru/_races/171007_V/171007_res_VUZ.htm',
        owner: 'SFSO_SPB',
        o_site: '171007_V'
    },
    {
        date: '2017-10-07',
        place: 'Зеленогорск',
        name: 'Чемпионат и п-во СПб - спорт глухих',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=801',
        res: 'https://o-site.spb.ru/_races/171007_D/171007_res_gl.htm',
        owner: 'SFSO_SPB',
        o_site: '171007_D'
    },
    {
        date: '2017-10-08',
        place: 'Петяярви',
        name: '59-й Чемпионат и первенство СПб - лонг',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=796',
        res: 'https://o-site.spb.ru/_races/171008/171008_res_.htm',
        owner: 'SFSO_SPB',
        o_site: '171008',
        reskeep: 243
    },
    {
        date: '2017-10-15',
        place: 'Кавголово (Орлиная гора)',
        name: 'Петербургская осень',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=802',
        res: 'https://o-site.spb.ru/_races/171014_Az/171015_res__.htm',
        info: 'Традиционные осенние соревнования КСО "Азимут"',
        logo: 'azimut.gif',
        o_site: '171014_Az'
    },
    {
        date: '2017-10-18',
        place: 'Школа №133, Красногвардейский район, ул. Петра Смородина, д.10А',
        name: 'Спортивное ориентирование - в школу (Красногвардейский район)',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/171018/171018_res_lab.htm',
        info: 'Проект Федерации Спортивного Ориентирования Санкт-Петербурга, направленный на популяризацию спортивного ориентирования среди школьников. Проходит при поддержке: Комитета по молодежной политике и взаимодействию с общественными организациями Санкт-Петербурга, Комитета по образованию Санкт-Петербурга, ОАО "Фацер"',
        owner: 'SFSO_SPB',
        o_site: '171018'
    },
    {
        date: '2017-10-22',
        name: 'Золотая осень',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/17102122/171021_res.htm',
        owner: 'VYBORG',
        o_site: '17102122'
    },
    {
        date: '2017-10-29',
        name: 'Яркий Кубок 2017',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/2017YC/2017YC_all_etaps.zip',
        photo: 'http://vk.com/club128591100',
        info: '1 этап - 08.05 - оз.Журавлево, лонг 2 этап - 14.05 - п.Гарболово, средняя 3 этап - 12.06 - п.Мичуринское, городской спринт 4 этап - 02.07 - п.Гарболово, средняя 5 этап - 19.08 - Первомайское, кросс 6 этап - 20.08 - Первомайское, средняя 7 этап - 02.09 - Орехово, кросс, общ.старт финал - 29.10 - Первомайское, лонг, гандикап',
        o_site: '2017YC'
    },
    {
        date: '2017-10-30',
        endDate: '2017-11-03',
        name: 'ТМ в ОК УНИВЕРСИТЕТСКИЙ',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/171103/171103_res.htm',
        o_site: '171103'
    },
    {
        date: '2017-11-02',
        name: 'Sochi-2017',
        type: 'ORIENT',
        res: 'http://orgeo.ru/live/#/5494',
        o_site: '171103Sochi'
    },
    {
        date: '2017-11-04',
        place: 'Поляны',
        name: 'Осенний марафон',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=809',
        res: 'https://o-site.spb.ru/_races/171004_OM/171104_res_.htm',
        info: 'Традиционные праздничные соревнования КСО "Азимут"',
        logo: 'azimut.gif',
        o_site: '171004_OM'
    },
    {
        date: '2017-11-05',
        name: 'Кубок Всеволожского района по рогейну - финал',
        type: 'ROGAINE',
        reg: 'http://sport-orient.ru/products/sorevnovaniya-po-rogeynu/kvr-2017-vtoroy-etap',
        res: 'https://o-site.spb.ru/_races/171105/171105_res.htm',
        o_site: '171105'
    },
    {
        date: '2017-11-05',
        place: 'Всеволожск, 1-е Ждановское озеро',
        name: 'Старт памяти 7 ноября',
        type: 'ORIENT',
        reg: 'http://sport-orient.ru/products/uncategorized/7-noyabrya-2017',
        res: 'https://o-site.spb.ru/_races/171107/171105_res2.htm',
        o_site: '171107'
    },
    {
        date: '2017-11-09',
        name: 'Чемпионат России в Севастополе',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/171109Sev/171109_res.htm',
        o_site: '171109Sev'
    },
    {
        date: '2017-11-12',
        place: 'Лемболово',
        name: 'МАРКИР-110',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=813',
        res: 'https://o-site.spb.ru/_races/171112_markir/171112_res1.html',
        start: 'ROGAINE_110',
        o_site: '171112_markir'
    },
    {
        date: '2017-11-12',
        place: 'Лесопарк «Сестрорецкие Дюны» (ул. Таможенная дорога)',
        name: 'Памяти Зои Викторовны Пятаковой',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=812',
        res: 'https://o-site.spb.ru/_races/171112/171112_res.htm',
        info: 'ПЕРВЕНСТВО СДЮСШОР КУРОРТНОГО Р-НА САНКТ-ПЕТЕРБУРГА ПО СПОРТИВНОМУ ОРИЕНТИРОВАНИЮ ПАМЯТИ ЗОИ ВИКТОРОВНЫ ПЯТАКОВОЙ',
        o_site: '171112'
    },
    {
        date: '2017-11-18',
        name: 'ГорныйКросс-60',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/171118R/171118R_res.pdf',
        fmt: '7 и 3 часа - бег, 6 часов - вело',
        o_site: '171118R'
    },
    {
        date: '2017-11-25',
        place: 'парк Сосновка',
        name: 'Ориентир НатАлка',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=815',
        res: 'https://o-site.spb.ru/_races/2017_Orientir/171125_res.htm',
        info: 'Традиционный тренировочный старт, проводимый КСО "Азимут"',
        logo: 'azimut.gif',
        o_site: '2017_Orientir'
    },
    {
        date: '2017-11-26',
        name: 'Снежная тропа, 3-й этап 2017',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=818',
        res: 'https://o-site.spb.ru/_races/171126/171126_res.htm',
        photo: 'https://o-site.spb.ru/_races/171126/171126_photos.rar',
        owner: 'SFSO_SPB',
        o_site: '171126'
    },
    {
        date: '2017-12-02',
        endDate: '2017-12-03',
        place: 'Санкт-Петербург',
        name: 'Семинар подготовки судей 2 и 3 категорий',
        type: 'ORIENT',
        info: 'Региональный семинар по подготовке и аттестации судей 2 и 3-й категорий по спортивному ориентированию для представителей Санкт-Петербурга и Ленинградской области. Программой мероприятия запланировано выступление лекторов из ФСОР. Проводится совместно со Всероссийским семинаром по подготовке спортивных судей Всероссийской и 1-й категории Внимание: прохождение семинара не дает автоматически категорию спортивного судьи. Желающим после прохождения семинара получить судейскую категорию или повысить имеющуюся, нужно заполнить представление и карточку учета судейской деятельности. В приложении квалификационные требования к спортивным судьям и НОВЫЕ представление и карточка учета. Консультацию можно получить у Васильевой Ирины, mk@spbof.ru',
        o_site: '17120203'
    },
    {
        date: '2017-12-03',
        name: 'Цикл стартов МаТрёшка',
        type: 'ORIENT',
        res: 'http://o-site.spb.ru/_races/170416/170416_rez_MT.htm',
        info: 'Цикл тренировочных стартов "МаТрёшка" от клуба NordWest 1-этап - 16.04.17 - Охта-парк Результаты Сплиты 2-этап - 30.04.17 - Сестрорецкие Дюны Результаты Сплиты 4-этап - 09.09.17 - Вартемяги Результаты Сплиты 5-этап - 30.09.17 - Пухтолова гора Результаты Сплиты 6-этап - 29.10.17 - Шуваловский парк Результаты Сплиты.',
        o_site: '170416'
    },
    {
        date: '2017-12-10',
        name: 'Всеволожская тропа 2017 5 этап',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/171119_VT/171210_rez.htm',
        o_site: '171119_VT'
    },
    {
        date: '2017-12-16',
        place: 'ФМЛ №239',
        name: 'Вечер встречи ориентировщиков',
        type: 'ORIENT',
        info: 'Традиционный вечер встречи по итогам летнего сезона 2017. Организатор: Федерация спортивного ориентирования Санкт-Петербурга Проходит при поддержке: Компании RedFox Сети магазинов «Спортивная Линия» Экипировочного центра «ОХТА» Экипировочного центра «Mass Sport» Кондитерской фабрики "Вереск" SIGN SPORT Russia В программе: Награждение (дипломами) за летний Чемпионат и Первенство СПб 2017 (велосипедные дисциплины), список призёров Награждение (дипломами) за летний Чемпионат и Первенство СПб 2017 (кроссовые дисциплины), Награждение по итогам летнего Кубка СПб и ЛО в личном и клубном зачётах, список призёров Место проведения: школа №239 Начало вечера: в 17:00 Все пришедшие на вечер участники награждения и гости смогут принять участие в лотерее, в которой будут разыграны призы от спонсоров. Внимание! С собой необходимо иметь сменную обувь или бахилы!',
        o_site: '171216'
    },
    {
        date: '2017-12-17',
        name: 'Снежная тропа, 4-й этап',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/171217/171217_res.htm',
        info: 'размещаются в средней общеобразовательной школе № 473 Калининского района по адресу: ул. Верности, д. 28, корп.',
        owner: 'SFSO_SPB',
        o_site: '171217'
    },
    {
        date: '2017-12-23',
        place: 'пос. Воейково',
        name: 'Weekly Orient, 3 этап',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=799',
        res: 'http://o-site.spb.ru/_races/2017_W_Or/170926_res.htm',
        info: '1-й этап: результаты, сплиты 2-й этап: результаты, сплиты',
        o_site: '2017_W_Or'
    },
    {
        date: '2017-12-30',
        place: 'Сертолово',
        name: 'Новогодняя Эстафета',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=825',
        res: 'http://reg.o-time.ru/result/17173.pdf',
        o_site: '171230'
    },
    {
        date: '2017-12-31',
        place: 'парк ЛТА',
        name: 'Тренировка FoxTeam',
        type: 'ORIENT',
        reg: 'http://o-reg.spb.ru/entry-list?id=828',
        res: 'https://o-site.spb.ru/_races/171231_/171231_res.htm',
        o_site: '171231_'
    }
];
