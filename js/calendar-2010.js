let events2010 = [
    {
        date: '2010-01-08',
        endDate: '2010-01-09',
        place: 'Рощино',
        name: 'Две Пятёрочки',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/10010809/10010809_res.htm',
        o_site: '10010809'
    },
    {
        date: '2010-01-17',
        name: 'На призы газеты Всеволожские Вести',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100117_vv/100117_res.htm',
        start: 'VSEV_VESTI',
        o_site: '100117_vv'
    },
    {
        date: '2010-01-17',
        name: 'Спринтерский Марафон',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100117/100117_res.zip',
        info: 'Ориентирование бегом на заснеженном грунте',
        o_site: '100117'
    },
    {
        date: '2010-01-22',
        endDate: '2010-01-24',
        place: 'Цвелодубово',
        name: 'Чемпионат и пер-во ЛО',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/10012224/100122_res.htm',
        info: 'Чемпионат и Первенство Ленинградской области по спортивному ориентированию на лыжах',
        owner: 'SFSO_LO',
        o_site: '10012224'
    },
    {
        date: '2010-01-23',
        place: 'Зеленогорск',
        name: 'Горный Кросс-18',
        type: 'ROGAINE',
        res: 'https://o-site.spb.ru/_races/100123/100123_res.htm',
        photo: 'http://fotki.yandex.ru/users/gornycross/album/88570',
        start: 'GORNY_CROSS',
        o_site: '100123'
    },
    {
        date: '2010-01-30',
        name: 'Эстафетный кубок «Невская Карусель»',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100130/100130_res.htm',
        o_site: '100130'
    },
    {
        date: '2010-01-31',
        place: 'Цвелодубово',
        name: 'Кубок Марафонов, Кубок центра «Ладога»',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100131/100131_res.htm',
        o_site: '100131'
    },
    {
        date: '2010-02-06',
        place: 'ж/д ст. Орехово. База отдыха Орехово (ЛАО)',
        name: 'МЕМОРИАЛ АНДРЕЯ ВОЛКОВА',
        type: 'SKI',
        res: 'https://o-site.spb.ru/_races/100206/100206_res_it.pdf',
        start: 'VOLKOV_A',
        o_site: '100206'
    },
    {
        date: '2010-02-06',
        endDate: '2010-02-07',
        place: 'Цвелодубово',
        name: 'Зимняя гонка Кант',
        type: 'ORIENT',
        res: 'http://www.adventureraces.ru/races/wr/2010/results.htm',
        o_site: '10020607'
    },
    {
        date: '2010-02-07',
        place: 'ж/д ст. Орехово. База отдыха Орехово (ЛАО)',
        name: 'Чемпионат и Первенство СПб, маркир',
        type: 'ORIENT',
        owner: 'SFSO_SPB',
        o_site: '100207'
    },
    {
        date: '2010-02-09',
        endDate: '2010-02-10',
        name: 'Первая Зимняя Универсиада Санкт-Петербурга',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/10020809/100209_split.htm',
        o_site: '10020809'
    },
    {
        date: '2010-02-13',
        place: 'ж/д ст. Орехово. База отдыха Орехово (ЛАО)',
        name: 'Чемпионат и Первенство СПб, эстафета и спринт',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/10021314/100213_res_it.pdf',
        info: '13 февраля. Эстафета на маркированной трассе. 14 февраля. Спринт, заданное направление',
        owner: 'SFSO_SPB',
        o_site: '10021314'
    },
    {
        date: '2010-02-21',
        endDate: '2010-02-22',
        place: 'Орехово',
        name: 'Чемпионат и п-во Лен.области. Кубок СПб',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/10022122_LO/100221_res.htm',
        info: '21 февраля - маркир; 22 февраля - заданное напр.',
        owner: 'SFSO_SPB',
        o_site: '10022122_LO'
    },
    {
        date: '2010-02-22',
        endDate: '2010-03-03',
        place: 'Зеркальный',
        name: 'УТС в Зеркальном',
        type: 'ORIENT',
        o_site: '100222_UTS'
    },
    {
        date: '2010-02-23',
        place: 'Орехово',
        name: 'Невская карусель - 3 этап',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100223_LO/100223_res.htm',
        info: 'Эстафета, заданное направление.',
        o_site: '100223_LO'
    },
    {
        date: '2010-02-28',
        place: 'ЗЦ Зеркальный, пл. 86 км',
        name: 'Кубок ЗЦ Зеркальный',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100228/100228_rez.htm',
        info: 'Лыжная гонка 45 мин.',
        o_site: '100228'
    },
    {
        date: '2010-03-06',
        endDate: '2010-03-07',
        name: 'Рогейн «100 за 24»',
        type: 'ROGAINE',
        res: 'https://o-site.spb.ru/_races/10030607/10030607_res6.htm',
        start: '100x24',
        o_site: '10030607'
    },
    {
        date: '2010-03-07',
        place: 'в районе пос. Комарово, на берегу озера Щучье',
        name: 'Чемпионат и Первенство СПб, классика и эстафета',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100307/100306_res.pdf',
        info: '06 марта. лыжная гонка 60 (классика). 07 марта. Эстафета, заданное направление.',
        owner: 'SFSO_SPB',
        o_site: '100307'
    },
    {
        date: '2010-03-08',
        place: 'Комарово, оз. Щучье',
        name: 'Мартовская восьмёрка',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100308_Y/100308_res.htm',
        info: 'Спринт в заданном направлении, входит в "Кубок ЗН"',
        o_site: '100308_Y'
    },
    {
        date: '2010-03-14',
        place: 'Павловский парк',
        name: 'Снежная Тропа, 2-й этап',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100315/100314_res.pdf',
        owner: 'SFSO_SPB',
        o_site: '100315'
    },
    {
        date: '2010-03-21',
        name: 'Кубок ОК Гольфстрим, этап 3',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100321/100321_res.htm',
        info: 'Программа зимней части Кубка ОК Гольфстрим: 1-й этап: 13 марта - спринт 2-й этап: 14 марта - удлинённая дистанция 3-й этап: 21 марта - маркир, классика',
        o_site: '100321'
    },
    {
        date: '2010-03-27',
        place: 'южные окрестности Петербурга',
        name: 'MultiSprint Cup',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100327/100327_res.htm',
        o_site: '100327'
    },
    {
        date: '2010-04-01',
        name: 'Летний Кубок СПб и ЛО 2010',
        type: 'ORIENT',
        info: 'Многоэтапные лично-командные соревнования по спортивному ориентированию «Кубок Санкт-Петербурга и Ленинградской области» проводятся Федерациями спортивного ориентирования Санкт-Петербурга и Ленинградской области с целью развития клубного движения и выявления сильнейших спортсменов-ориентировщиков и сильнейших клубов спортивного ориентирования СПб и ЛО. этап 1 18.04 рейтинг этап 2 25.04 рейтинг этап 3 02.05 рейтинг этап 4 15.05 рейтинг этап 5 29.05 рейтинг этап 6 05.06 рейтинг этап 7 19.09 рейтинг этап 8 22.09 рейтинг этап 9 03.10 рейтинг этап 10 24.10 рейтинг',
        o_site: 'SPb_LO_Cup',
        endDate: '2010-10-31'
    },
    {
        date: '2010-04-04', name: 'Вечер встречи ориентировщиков', type: 'ORIENT', o_site: '100404'
    },
    {
        date: '2010-04-10',
        place: 'Орехово',
        name: 'Горный Кросс-19',
        type: 'ROGAINE',
        start: 'GORNY_CROSS',
        o_site: '100410'
    },
    {
        date: '2010-04-10',
        place: 'Московский парк Победы',
        name: 'Тренировка от Стрекозаек (10 апреля (суббота))',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100410_st/100410_res.htm',
        info: '10 апреля команда Стрекозайки проводит открытую тренировку в Московском парке Победы',
        o_site: '100410_st'
    },
    {
        date: '2010-04-11',
        name: 'Всеволожская тропа',
        type: 'ORIENT',
        res: 'http://www.o-site.spb.ru/_races/100411_tropa/091129_res1.htm',
        info: '1 этап 29 ноября Результаты 2 этап 13 декабря Результаты 3 этап 28 февраля Результаты 4 этап 08 марта Результаты',
        o_site: '100411_tropa'
    },
    {
        date: '2010-04-17',
        endDate: '2010-04-18',
        place: 'Сосновый Бор',
        name: 'Сосновоборские дюны',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100417_sb/100417_res.htm',
        info: 'Традиционные соревнования, открывающие летний О-сезон в Санкт-Петербурге и Ленинградской области. 2-й день соревнований идёт в зачёт многоэтапного лично-командного Кубка Санкт-Петербурга и Ленинградской области 2010.',
        owner: 'YUVENTA',
        o_site: '100417_sb'
    },
    {
        date: '2010-04-21',
        place: 'Токсово',
        name: 'Ориентир-8 (15 сентября 2010',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100421_OR/100915_res.htm',
        info: 'Тренировочный старт КСО Азимут.',
        o_site: '100421_OR'
    },
    {
        date: '2010-05-01',
        endDate: '2010-05-03',
        place: 'Цвелодубово',
        name: 'Чемпионат и пер-во ЛО',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/10050103/100501_res.htm',
        owner: 'SFSO_LO',
        o_site: '10050103'
    },
    {
        date: '2010-05-08',
        place: 'г. Пухтола',
        name: 'Соревнования, посвящённые Дню Победы',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100508_Az/100508_res.htm',
        info: 'Соревнования, посвящённые 65-й годовщине Победы Открытое п-во СДЮСШОР Курортного р-на, посв. Дню Победы',
        fmt: 'Классика40',
        o_site: '100508_Az'
    },
    {
        date: '2010-05-09',
        place: 'Юкки',
        name: 'Эстафета 2х2',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100508_2x2/100509_res.htm',
        o_site: '100508_2x2'
    },
    {
        date: '2010-05-15',
        place: 'Комарово (слева от ж/д)',
        name: 'Клубные эстафеты (2+1)',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100515_est/100515_res.htm',
        info: 'Эстафеты (2+1), входящие в клубный зачёт Кубка СПб и ЛО: МЖ14, МЖ18, МЖ21, а также ветеранские группы «сумма возрастов ≥120» (участники старше 35 лет) и «сумма возрастов ≥150» (участники старше 45 лет).',
        o_site: '100515_est'
    },
    {
        date: '2010-05-16',
        place: 'г.Всеволожск',
        name: 'Российский Азимут ЛО',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/10RA_LO/10RA_LO_split.htm',
        fmt: 'дистанция по выбору',
        owner: 'SFSO_SPB',
        o_site: '10RA_LO'
    },
    {
        date: '2010-05-16',
        place: 'Комарово',
        name: 'Спринт Кубок по Рогейну, 2-й этап',
        type: 'ROGAINE',
        res: 'https://o-site.spb.ru/_races/100403/100516_res.htm',
        fmt: '3 часа',
        o_site: '100403'
    },
    {
        date: '2010-05-22',
        place: 'Лемболово (у родника)',
        name: 'Памяти друзей (22 мая (суббота))',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100522_Az/100522_res.htm',
        info: 'Традиционный старт клуба Азимут.',
        o_site: '100522_Az'
    },
    {
        date: '2010-05-22',
        endDate: '2010-05-23',
        place: 'Селгисе, Эстония',
        name: 'Чемпионат Прибалтики',
        type: 'ORIENT',
        res: 'http://www.okilves.ee/voist/baltimv2010/index.php',
        photo: 'http://o-site.spb.ru/gallery.php?id=gallery83',
        info: 'Выезд сборной команды Петербурга на чемпионат Прибалтики в период 21-23 мая 2010 г. в Селгисе, Эстония (40 км к северо-востоку от Тарту).',
        o_site: '10052223'
    },
    {
        date: '2010-05-23',
        place: 'Осиновая роща',
        name: 'Первенство Санкт-Петербурга среди обучающихся',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100523/100523_res.htm',
        owner: 'SFSO_SPB',
        o_site: '100523'
    },
    {
        date: '2010-05-29', name: 'MatrasOFF Race', type: 'ORIENT', o_site: '100529'
    },
    {
        date: '2010-05-29',
        place: 'пос. Сосново (гора Каланчевка)',
        name: 'Чемпионат СПб, классика',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100529_1/100529_res.htm',
        info: '52-й ЛЕТНИЙ ЧЕМПИОНАТ и ПЕРВЕНСТВО САНКТ-ПЕТЕРБУРГА по СПОРТИВНОМУ ОРИЕНТИРОВАНИЮ БЕГОМ, классическая дистанция.',
        owner: 'SFSO_SPB',
        o_site: '100529_1',
        reskeep: 250
    },
    {
        date: '2010-05-29',
        endDate: '2010-05-30',
        place: 'Кузнечное',
        name: 'Рогейн «100 за 24»',
        type: 'ROGAINE',
        res: 'https://o-site.spb.ru/_races/10052930/10052930_res.zip',
        fmt: '3, 6 и 24 часа',
        start: '100x24',
        o_site: '10052930'
    },
    {
        date: '2010-05-30',
        place: 'Колосково (пл. 78 км)',
        name: 'Первенство Команды 110%',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100530/100530_res.htm',
        start: 'ROGAINE_110',
        o_site: '100530'
    },
    {
        date: '2010-05-30',
        place: 'Сосновский парк',
        name: 'Российский Азимут СПб',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/10RA_SPb/100530_res.htm',
        owner: 'SFSO_SPB',
        o_site: '10RA_SPb'
    },
    {
        date: '2010-06-04', name: 'Весенний Кубок команды 110%', type: 'ORIENT', start: 'ROGAINE_110', o_site: '100604'
    },
    {
        date: '2010-06-05',
        place: 'Токсово',
        name: 'Velodrive O-cup, 3-й этап',
        type: 'ORIENT',
        res: 'http://o-time.ru/content/view/479/28/',
        o_site: '100508'
    },
    {
        date: '2010-06-05',
        place: 'Комарово',
        name: 'Чемпионат СПб, спринт',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100605_SPb/100605_res_.htm',
        info: '52-й летний чемпионат и первенство Санкт-Петербурга по спортивному ориентированию',
        owner: 'SFSO_SPB',
        o_site: '100605_SPb',
        reskeep: 262
    },
    {
        date: '2010-06-06',
        place: 'Токсово, ВИФК',
        name: 'Чемпионат СПб, эстафеты',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100606/100606_res.htm',
        owner: 'SFSO_SPB',
        o_site: '100606'
    },
    {
        date: '2010-06-09', endDate: '2010-06-19', place: 'Петяярви', name: 'УТС ЛО', type: 'ORIENT', o_site: '10060919'
    },
    {
        date: '2010-06-12',
        place: 'Петяярви',
        name: 'Кубок «Центра Ладога», финал',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100612/100612_res.html',
        o_site: '100612'
    },
    {
        date: '2010-06-12',
        endDate: '2010-06-14',
        place: 'Мичуринское',
        name: 'Невский Азимут',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/10061214_na/100612_res.htm',
        info: 'ХХ традиционные многодневные соревнования Невский Азимут Открытое первенство СДЮСШОР Курортного р-на им. В.А.Коренькова Командный кубок СДЮСШОР Курортного р-на им. В.А.Коренькова',
        start: 'NA',
        o_site: '10061214_na'
    },
    {
        date: '2010-06-15',
        endDate: '2010-06-20',
        place: 'п. Мичуринское Приозерского района ЛО',
        name: 'Фестиваль «Белые Ночи-2010»',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/10061520/10061520_res.zip',
        owner: 'WN',
        start: 'WN',
        o_site: '10061520'
    },
    {
        date: '2010-06-19',
        name: '2-й этап вело-Кубка по рогейну',
        type: 'ROGAINE',
        res: 'http://www.100x24.ru/2010/results/20100619_prelim.htm',
        o_site: '100619'
    },
    {
        date: '2010-06-21',
        endDate: '2010-06-23',
        place: 'Лемболово',
        name: 'Карельские Скалы',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/10062123_KS/KS2010_split.zip',
        start: 'KS',
        o_site: '10062123_KS'
    },
    {
        date: '2010-06-25',
        endDate: '2010-06-27',
        place: 'Лемболово',
        name: 'Мемориал Михаила Святкина',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/10062527/100625_res.htm',
        start: 'MMS',
        o_site: '10062527'
    },
    {
        date: '2010-07-03',
        endDate: '2010-07-05',
        place: 'Приозерск - Кузнечное',
        name: 'Кубок России по ориентированию бегом, Приозерск',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/10070305/100703_res.pdf',
        info: '3 июля - Спринт (Приозерск) 4 июля - Классика (Кузнечное) 5 июля - Кросс (Кузнечное)',
        o_site: '10070305'
    },
    {
        date: '2010-07-10',
        place: 'Токсово',
        name: 'Чемпионат ЛО по рогейну',
        type: 'ROGAINE',
        res: 'https://o-site.spb.ru/_races/100710/100710_res.htm',
        fmt: '3 часа',
        o_site: '100710'
    },
    {
        date: '2010-07-11',
        place: 'Орехово',
        name: 'Открытое п-во ОК судостроения',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100711_Az/100711_res.htm',
        info: 'Традиционный старт КСО Азимут и ориентировщиков-судостроителей',
        o_site: '100711_Az'
    },
    {
        date: '2010-07-17',
        endDate: '2010-07-18',
        place: 'Лемболово',
        name: 'Горный Кросс',
        type: 'ROGAINE',
        res: 'https://o-site.spb.ru/_races/10071718/100717_res.xls',
        start: 'GORNY_CROSS',
        o_site: '10071718'
    },
    {
        date: '2010-07-24',
        endDate: '2010-07-25',
        name: 'Три-О-Тлон',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/tri-o-tlon-2010/TOT_res.htm',
        photo: 'http://o-site.spb.ru/gallery.php?id=gallery90&page=1',
        info: 'Старейшая мультиспортивная гонка в России: в 2010-м году состоялся уже 9-й Три-О-Тлон, история которого ведёт отсчёт с 2002-го года. Самая качественная (по организации) гонка 2009-го года согласно опросу на сайте x-race.msk.ru Классическая гонка по набору дисциплин: треккинг, велосипед, байдарка, верёвочные этапы. Основная направленность гонки - навигация. Три дистанции - длинная, средняя и короткая. Участвуют команды из 2-х человек любого пола. Нас поддержали: СК Темп Альпина Спортивная Линия Яркий Мир Маноми Кант Спортсистем Про-Спорт Альпиндустрия Эксперты Приключений Информационная поддержка: O-site x-race.msk.ru adventureraces.ru carabin.ru',
        logo: 'triotlon.gif',
        o_site: 'tri-o-tlon-2010'
    },
    {
        date: '2010-08-08',
        place: 'Лемболово',
        name: 'Рогейн-110',
        type: 'ROGAINE',
        res: 'https://o-site.spb.ru/_races/100808/100808_res.xls',
        start: 'ROGAINE_110',
        o_site: '100808'
    },
    {
        date: '2010-08-21',
        endDate: '2010-08-22',
        name: 'PITERHAPPENING',
        type: 'ORIENT',
        res: 'http://piterhappening.ru/100818_res.htm',
        info: 'Мультиспортивная гонка от Виталия Чегаровского',
        o_site: '10082122'
    },
    {
        date: '2010-08-29',
        place: 'Лемболово, оз.Горынское',
        name: 'Зелёный змей',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100829_Az/100829_res.htm',
        info: 'Традиционный августовский старт КСО Азимут',
        start: 'GS',
        o_site: '100829_Az'
    },
    {
        date: '2010-09-04',
        name: 'Велодрайв О-Cup, Финал',
        type: 'VELO',
        res: 'https://o-site.spb.ru/_races/100718/100904_res.htm',
        info: 'V этап Велодрайв-О-Cup',
        o_site: '100718'
    },
    {
        date: '2010-09-10',
        endDate: '2010-09-15',
        name: 'КУБОК РОССИИ по спортивному ориентированию на велосипедах',
        type: 'VELO',
        res: 'https://o-site.spb.ru/_races/10091015/100910_res.htm',
        info: 'КУБОК РОССИИ по спортивному ориентированию на велосипедах ПЕРВЕНСТВО РОССИИ по спортивному ориентированию на велосипедах (юноши, девушки до 18 лет) RUSSIAN MBO CUP RUSSIAN YOUTH MBO CHAMPIONCHIP PRE EUROPEAN MBO CHAMPIONSHIP-2011 OPEN EVENTS Leningrad Region, Orekhovo, 10-15. 09. 2010',
        o_site: '10091015'
    },
    {
        date: '2010-09-11',
        place: 'пос. Молодёжное',
        name: 'Горный Кросс-22',
        type: 'ROGAINE',
        res: 'https://o-site.spb.ru/_races/100911/100911_res.xls',
        fmt: '3 и 6 часов',
        start: 'GORNY_CROSS',
        o_site: '100911'
    },
    {
        date: '2010-09-12',
        place: 'Комарово',
        name: 'Классика Белых Ночей',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100912/100912_res.zip',
        owner: 'WN',
        o_site: '100912'
    },
    {
        date: '2010-09-18',
        place: 'Ушково (г.Пухтола)',
        name: 'Открытое п-во КСО Азимут (18 сентября (суббота))',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100918_Az/100918_res.htm',
        logo: 'azimut.gif',
        o_site: '100918_Az'
    },
    {
        date: '2010-09-19',
        place: 'Мичуринское',
        name: 'Чемпионат СПб, марафон',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/100919/100919_split-1.htm',
        owner: 'SFSO_SPB',
        o_site: '100919',
        reskeep: 241
    },
    {
        date: '2010-09-22',
        place: 'Пискаревский парк',
        name: 'Парковый спринт - этап Кубка СПб и ЛО',
        type: 'ORIENT',
        res: 'http://o-time.ru/images/all/s10/footo220910.htm',
        o_site: '100922'
    },
    {
        date: '2010-09-24',
        endDate: '2010-09-27',
        place: 'Зеркальный',
        name: 'Юношеская многодневка в Зеркальном - Марафон На Холмах Яппиля',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/10092427/10092427_res.zip',
        info: 'традиционная юношеская осенняя многодневка в рамках смены Загородного Центра «Зеркальный»',
        o_site: '10092427'
    },
    {
        date: '2010-10-02',
        place: 'Гарболово',
        name: 'Золотые Кольца',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/101002/101002_res.xls',
        start: 'WEDDING',
        o_site: '101002'
    },
    {
        date: '2010-10-03',
        place: 'Шапки',
        name: 'Чемпионат и Первенство СПб, средняя дистанция',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/101003/101003_res.htm',
        owner: 'SFSO_SPB',
        o_site: '101003',
        reskeep: 229
    },
    {
        date: '2010-10-07',
        place: 'Кузьмолово',
        name: 'Старт памяти Коли Иванова',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/101007/101007_res.htm',
        o_site: '101007'
    },
    {
        date: '2010-10-09',
        endDate: '2010-10-10',
        place: 'Зеленогорск, Чёрные Озёра',
        name: 'XXXIX соревнования по ночному ориентированию памяти А.Окинчица',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/101009/101009_res.htm',
        start: 'OKINCHITSA',
        o_site: '101009'
    },
    {
        date: '2010-10-09',
        place: 'оз.Щучье, пос Комарово',
        name: 'Чемпионат и Первенство ЛО',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/10100910/10100910_res.zip',
        owner: 'SFSO_LO',
        o_site: '10100910',
        endDate: '2010-10-10'
    },
    {
        date: '2010-10-16',
        name: 'Кубок Яркого Мира 2010',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/2010CupYM/2010CupYM_res_public.zip',
        info: '1 этап - 24.04 детские Дюны 2 этап - 25.04 Дюны 3 этап - 10.05 оз.Банное 4 этап - 04.07 оз.Медное 5 этап - 21.08 Мичуринское 6 этап - 22.08 Мичуринское 7 этап - 04.09 оз.Пасторское 8 этап - 05.09 оз.Пасторское 9 этап - 19.09 Мичуринское ком.финал - 16.10 Воейково, эстафета-гандикап лич.финал - 17.10 Воейково, гандикап',
        start: 'YM',
        o_site: '2010CupYM'
    },
    {
        date: '2010-10-16',
        place: 'пос. Песочный Курортного р-на',
        name: 'Ночные Эстафеты',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/101016/101016_res.htm',
        o_site: '101016'
    },
    {
        date: '2010-10-16',
        endDate: '2010-10-17',
        place: 'Орехово',
        name: 'Кубок ОК Гольфстрим, финальные этапы',
        type: 'ORIENT',
        res: 'http://o-site.spb.ru/_races/100314/100313_res.htm',
        info: '16-17.10 – финальный блок этапов кубка в Орехово - ЛАО. 2 этапа вело ориентирования и 3-й этап бегового кубка – ночное ориентирование. Прошедшие этапы Кубка ОК Гольфстрим: 1-й лыжный этап: 13 марта - спринт результаты 2-й лыжный этап: 14 марта - удлинённая дистанция результаты 3-й лыжный этап: 21 марта - маркир, классика результаты 1-й беговой этап: 26 сентября – Марафон на холмах Яппиля результаты 2-й беговой этап: 29 сентября - спринт в Сосновке результаты',
        o_site: '100314'
    },
    {
        date: '2010-10-23',
        endDate: '2010-10-24',
        name: 'Золотая Осень, Финал Кубка СПб и ЛО',
        type: 'ORIENT',
        res: 'http://orientirclub.ucoz.ru/index/rezultaty_1_go_dnja/0-5',
        owner: 'VYBORG',
        o_site: '10102324'
    },
    {
        date: '2010-10-30',
        place: 'Лемболово',
        name: 'Критериум',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/101030/101030_res.htm',
        o_site: '101030'
    },
    {
        date: '2010-10-31',
        place: 'Зеленогорск-Комарово',
        name: 'Петербургская осень, Первенство ВУЗов (31 октября 2010 (воскресенье))',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/101031_Az/101031_res.htm',
        info: 'Закрытие сезона КСО Азимут и СДЮСШОР Курортного р-на. Первенство СПб среди студентов ВУЗов',
        owner: 'SFSO_SPB',
        logo: 'azimut.gif',
        o_site: '101031_Az'
    },
    {
        date: '2010-11-01',
        endDate: '2010-11-07',
        place: 'лагерь «Мечта», пос. Мичуринское',
        name: 'УТС в «Мечте»',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/10110107/101106_res.zip',
        o_site: '10110107'
    },
    {
        date: '2010-11-01',
        endDate: '2010-11-07',
        place: 'Цвелодубово',
        name: 'УТС в Цвелодубово',
        type: 'VELO',
        res: 'https://o-site.spb.ru/_races/10110107_Zv/101104_res.htm',
        o_site: '10110107_Zv'
    },
    {
        date: '2010-11-04',
        place: 'Лемболово (слева от ж/д)',
        name: 'Осенний марафон',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/101104_Az/101104_res.htm',
        info: 'Традиционные праздничные ноябрьские соревнования на удлинённой дистанции, проводимы клубом Азимут',
        logo: 'azimut.gif',
        o_site: '101104_Az'
    },
    {
        date: '2010-11-05',
        endDate: '2010-11-06',
        place: 'п.Назия (ж/д ст. Жихарево)',
        name: 'Кубок СПб по рогейну',
        type: 'ROGAINE',
        res: 'http://rogaining.spb.ru/index.php?option=com_content&task=view&id=62&Itemid=122',
        info: '2 этап Большого Кубка и 3 этап Вело-Кубка. Открытый Чемпионат Санкт-Петерурга по классическому рогейну (24 часа).',
        fmt: 'Форматы 24, 8 и 3 часа; 8-ВЕЛО',
        o_site: '10110506'
    },
    {
        date: '2010-11-18',
        name: 'Осенний Кубок команды 110%',
        type: 'ORIENT',
        res: 'http://www.o-site.spb.ru/_races/101118/100916_res.htm',
        info: '16 сентября Кавголово Результаты 30 сентября Токсово Результаты 14 октября Юкки Результаты 21 октября Кузьмолово Результаты 11 ноября Кавголово Результаты 18 ноября Кузьмолово',
        start: 'ROGAINE_110',
        o_site: '101118'
    },
    {
        date: '2010-11-21',
        place: 'Пискарёвский парк',
        name: 'Снежная Тропа, 1-й этап',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/101121/101121_res1.htm',
        info: '21 ноября 2010 г. - 1-й этап 12 декабря 2010 г. - 2-й этап',
        fmt: 'ориентирование по выбору',
        owner: 'SFSO_SPB',
        o_site: '101121'
    },
    {
        date: '2010-11-27',
        place: 'Зеленогорск',
        name: 'Горный кросс-23',
        type: 'ROGAINE',
        res: 'https://o-site.spb.ru/_races/101127_gk/101127_res.xls',
        info: 'Рогейн. 6-й этап Кубка Горный кросс-2010',
        start: 'GORNY_CROSS',
        o_site: '101127_gk'
    },
    {
        date: '2010-11-28',
        place: 'Зеленогорск (г. Серенада)',
        name: 'Кубок ОК Гольфстрим, 1 этап',
        type: 'ORIENT',
        res: 'http://gs-oc.ru/phocadownload/competitions/gs-cup-2011/101128_results.htm',
        info: '1-й этап: 28 ноября – Маркированная трасса бегом 35-60 минут. Заявка производится на сайте http://o-reg.spb.ru/',
        o_site: '101128'
    },
    {
        date: '2010-12-05',
        place: 'Крестовский пр. 19',
        name: 'Вечер встречи ориентировщиков',
        type: 'ORIENT',
        info: 'Вечер встречи ориентировщиков состоиться в конференц–зале Эколого-биологического Центра «Крестовский остров», СДЮСШОР №2 ГОУ ЦО «СПбГДТЮ» Начало в 17:00',
        o_site: '101205'
    },
    {
        date: '2010-12-05',
        place: 'Песочная',
        name: 'Ориентир НатАлка',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/101205_Az/101205_res.htm',
        info: 'Традиционный старт КСО Азимут',
        logo: 'azimut.gif',
        o_site: '101205_Az'
    },
    {
        date: '2010-12-12',
        place: 'Парк Зверинец, Петродворец',
        name: 'Снежная Тропа, 2-й этап',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/101212/101212_res.htm',
        info: '21 ноября 2010 г. - 1-й этап 12 декабря 2010 г. - 2-й этап',
        owner: 'SFSO_SPB',
        o_site: '101212'
    },
    {
        date: '2010-12-25',
        place: 'Токсово, Малиновая Гора',
        name: 'Новогодний Рогейн',
        type: 'ROGAINE',
        res: 'http://o-time.ru/images/all/w11/rgn_ny.htm',
        o_site: '101225'
    },
    {
        date: '2010-12-26',
        place: 'Токсово, Малиновая Гора',
        name: 'Новогодняя Эстафета',
        type: 'ORIENT',
        res: 'https://o-site.spb.ru/_races/101226/101226_res.htm',
        o_site: '101226'
    },
];
